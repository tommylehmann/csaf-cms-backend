...
export default class DocumentEntity {
   ...
   */
  preview(document) {
    // Deep-clone so the caller's input object is never mutated (ADR-0003).
    // GraalVM's JS context has no lodash; the document is plain JSON-origin
    // data (helper lambdas are attached after cloning), so a structural JSON
    // clone is faithful and dependency-free.
    ...
    if (vulnerabilities) {
      for (let i = 0; i < vulnerabilities.length; ++i) {
        ...
        addProductStatusPreviewAttributes(
            vulnerability,
            productIds,
            templateDoc.product_tree?.product_groups
        )
...
/**
 * Retrieve the maximum baseScore of all metrics
 *
 ...
 *
 * @param {{metrics: [], product_status: any}} vulnerability
 * @param {*} productIds
 * @param {*} productGroups - accepted to match the upstream signature and the
 *   restored caller argument; unused in 2.0 because the per-product `flags`
 *   feature (upstream `addFlags`) is intentionally not ported here (Q1=minimal,
 *   keeps 2.0 output unchanged).
 */
const addProductStatusPreviewAttributes = (
    vulnerability,
    productIds,
    productGroups // eslint-disable-line no-unused-vars
) => {
